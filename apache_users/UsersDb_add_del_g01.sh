...
#    echo $n_user_corr
    ...
    EncryptedPassword=$(openssl passwd -crypt g01)
...
#    cmd="useradd -m -s /bin/bash -k ./folder -g "$user_pref" -d /var/www/"$user_pref"/"$user_name" "$user_name;    echo $cmd;    $cmd
...
#    $cmd
    ...
    cmd="chmod 777 /var/www/"$user_pref;echo $cmd;$cmd
...
#    cmd="chmod 711 /var/www/"$user_pref"/"$user_name"/*";echo $cmd;$cmd
    ...
    cmd="chmod 755 /var/www/"$user_pref"/"$user_name"/cgi-bin/*.*";echo $cmd;$cmd
...
#    cmd="chown root /var/www/"$user_pref"/"$user_name"/.htaccess";echo $cmd;$cmd
    ...
    cmd="ls -laF /var/www/"$user_pref"/"$user_name"/cgi-bin";echo $cmd;$cmd
